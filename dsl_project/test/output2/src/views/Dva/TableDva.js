...
const TableDva = () => {
  ...
  const [listDva, setListDva] = useState([]);
  ...
  const handleConfirmDelete = (id) => {
    const fetchData = async () => {
      try {
          const response = await dvaService.deleteDva(id);
          if (response.status === 200) {
              setListDva(response.data);
    ...
    <div>
      ...
      <TableContainer component={Paper}>
        <Table className={classes.table} aria-label="simple table">
          ...
          <TableBody>
            {listDva.map((item, index) => (
              <TableRow key={index}>
               <TableCell align="center">{index + 1}</TableCell>
                <TableCell align="center">{item.sds}</TableCell>
