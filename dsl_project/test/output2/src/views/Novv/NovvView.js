...
const NovvView = () => {
  ...
  const { id } = useParams();
  const [novv, setNovv] = useState(NaN)
    ...
    const fetchData = async () => {
      try {
          ...
          if (response.status === 200) {
              setNovv(response.data);
          }
          const responseCustomer = await novvService.getCustomersByNovv(id);
      } catch (error) {
          console.error(error);
      }
  };
