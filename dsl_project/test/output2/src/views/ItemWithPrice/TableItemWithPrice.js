...
const TableItemWithPrice = () => {

  const handleAdd = () => {
    navigate(`/add-itemwithprice`);
  };
  const handleView = (id) => {

    navigate(`/itemwithprice-view/`+id);
  };

  const navigate = useNavigate();
  const handleEdit = (id) => {
    navigate(`/edit-itemwithprice/${id}`);
  };
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [delId, setDelId] = useState(-1);
  const [listItemWithPrice, setListItemWithPrice] = useState([]);
  ...
  const handleConfirmDelete = (id) => {
    const fetchData = async () => {
      try {
          const response = await itemWithPriceService.deleteItemWithPrice(id);
          if (response.status === 200) {
              setListItemWithPrice(response.data);
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            const response = await itemWithPriceService.getItemWithPrice();
            if (response.status === 200) {
                setListItemWithPrice(response.data);
            }
        } catch (error) {
    ...
    <div>
      ...
      <TableContainer component={Paper}>
        <Table className={classes.table} aria-label="simple table">
          ...
          <TableBody>
            {listItemWithPrice.map((item, index) => (
              <TableRow key={index}>
...
export default TableItemWithPrice;