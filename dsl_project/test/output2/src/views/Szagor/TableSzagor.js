...
const TableSzagor = () => {
  ...
  const [listSzagor, setListSzagor] = useState([]);
  ...
  const handleConfirmDelete = (id) => {
    const fetchData = async () => {
      try {
          const response = await szagorService.deleteSzagor(id);
          if (response.status === 200) {
              setListSzagor(response.data);
