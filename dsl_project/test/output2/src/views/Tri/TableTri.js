...
const TableTri = () => {
  ...
  const [listTri, setListTri] = useState([]);
  ...
  const handleConfirmDelete = (id) => {
    const fetchData = async () => {
      try {
          const response = await triService.deleteTri(id);
          if (response.status === 200) {
              setListTri(response.data);
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            const response = await triService.getTri();
            if (response.status === 200) {
                setListTri(response.data);
            }
        } catch (error) {
        ...
        <Table className={classes.table} aria-label="simple table">
          <TableHead>
            <TableRow>
              ...
              <TableCell align="center">sdssdfe</TableCell>
            ...
            {listTri.map((item, index) => (
              <TableRow key={index}>
               <TableCell align="center">{index + 1}</TableCell>
                <TableCell align="center">{item.sdssdfe}</TableCell>
