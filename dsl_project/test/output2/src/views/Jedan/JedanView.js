...
const JedanView = () => {
  ...
  const { id } = useParams();
  const [jedan, setJedan] = useState(NaN)
    ...
    const fetchData = async () => {
      try {
          ...
          if (response.status === 200) {
              setJedan(response.data);
          }
          const responseCustomer = await jedanService.getCustomersByJedan(id);
      } catch (error) {
          console.error(error);
      }
  };
