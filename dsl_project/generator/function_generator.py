...
def generate_if_statement(statement, generated_code, def_var, param_list ):  # ovde moze biti problema sa get
    condition = re.sub(r'&&', ' && ', statement.conditions)
    condition = re.sub(r'\s*\|\|\s*', ' || ', condition)
    generated_code += " if( " + condition + " ){"
    generated_code += main_statement_generator(statement.if_statements, "", def_var, param_list)
    generated_code += "}"
    ...
    if statement.else_statements is not None:
        generated_code += "else{" + main_statement_generator(statement.else_statements, "", def_var, param_list) \
                          + "}"
...
def generate_variable_statement(statement, generated_code, main_statemant,definde_var, param_list  ):
    ...
    if isinstance(statement, NewObjectStatement):
        ...
        if main_statemant.type == "int" or main_statemant.type == "float" or main_statemant.type == "double":
            ...
            if name_var[-1] != "(":
                ...
                generated_code += generate_one_line_condition(statement, main_statemant.name, definde_var, param_list, main_statemant)
        return generated_code
        ...
        if not new_generated_text[-1] ==";" and not new_generated_text[-1] =="}":
            new_generated_text+=";"
        ...
        if type_var != "":
            generated_code += type_var + " " + main_statemant.name + "=" + generate_chane_code(
                statement, True) + ";"
            return generated_code
        if main_statemant ==None:
            generated_code += generate_chane_code(statement, True) + ";"
        else:
...
def condition_generator(statement):
    ...
    elif isinstance(statement.left_side, Condition):
        ls = condition_generator(statement.left_side)
    elif isinstance(statement.left_side, Param):
...
def generate_one_line_condition(statement, name, definde_var, param_list, main_stmt=None):
...
def generate_operation(statement):
    ...
    if isinstance(statement.right_side, Variable):
        right_side = statement.right_side.name
    else:
...
def short_func_generator(statement, name, block=0, inter_code=""):
    if isinstance(statement, ShortFunctions):
        if statement.name == "for":
            ...
            return generate_size_func(statement, name, block)
...
def generate_sap_func(statement, name, block=0, id="", operation="a"):
    ...
    elif isinstance(possible_list, Variable) or isinstance(possible_list, Param) and possible_list is not None:
...
def generate_all_any_none_func(statement, name, block=0, first="true", not_op="", second="false"):
    ...
    condition = re.sub(r'&&', ' && ', condition_generator(statement.where_part))
    condition = re.sub(r'\s*\|\|\s*', ' || ', condition)
    generated_code += "if(" + not_op + "(" + condition + ")){"
    generated_code += new_var + " = " + second + ";break;}}"

    generated_code += 'boolean ' + name + "=" + new_var + ";"
    return generated_code
...
def generate_min_max_func(statement, name, block=0, max_min=">"):
    ...
    if isinstance(as_part, ClassWithGeters):
        ...
        generated_code += "Object " + "new_var;if(" + possible_list + ".size()>0){" + new_var + " = " + possible_list + \
                          ".get(0);}else{" + new_var + " = null;}"
        generated_code += "for(Object " + statement.as_part.name + " : " + possible_list + " ){"
    elif isinstance(as_part, Variable):
        as_part_type = as_part.type
        generated_code += as_part.type+" "+ new_var +"; if(" + possible_list + ".size()>0){" + new_var + " = " + possible_list + \
                          ".get(0);}else{" + new_var + " = null;}"
        generated_code += "for("+as_part.type+" " + as_part.name + " : " + possible_list + " ){"
        as_part = as_part.name
    else:
        generated_code += "double "+ new_var +"; if(" + possible_list + ".size()>0){ " + new_var + " = " + possible_list + \
                          ".get(0);}else{ " + new_var + " = null;}"
        generated_code += "for(Object " + as_part + " : " + possible_list + " ){"
    if statement.where_part is not None:
        condition = re.sub(r'&&', ' && ', condition_generator(statement.where_part))
    ...
    generated_code += as_part_type + " " + name + "=" + new_var + ";"
    return generated_code


def generate_size_func(statement, name, block=0):
    pl = statement.posible_list
    generated_code = ""
    if isinstance(pl, ClassWithGeters):
        return "int "+ name + " = " + generate_chane_code(pl, True) + ".size()"
...
def generate_for_func(statement, name, block=0):  # todo ovo prodji ponovo, nije dobro
    ...
    if statement.for_func is not None:
        ...
        if statement.for_func == "product":
            ...
            inter_code = new_var + " = " + new_var + "*"
    generated_code += short_func_generator(statement.short_func, new_var, block + 1, inter_code) + "}"
...
def generate_chane_code(statement, get_or_set):
    if not get_or_set and len(statement.list_geters) == 1:
        if isinstance(statement.name, Param):
             generated_code = statement.name.name + ".set"
        else:
             generated_code = statement.name + ".set"
    else:
        if isinstance(statement.name, ClassWithGeters):
            generated_code = statement.name.name + ".get"
        else:
