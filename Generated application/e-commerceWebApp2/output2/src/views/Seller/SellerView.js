...
const SellerView = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
          ...
          const responseCustomer = await sellerService.getCustomersBySeller(id);
      } catch (error) {
          console.error(error);
      }
  };
  ...
  <div>
    <ToastContainer />
    {(userType=='ADMIN' || userType=='SELLER') && (<div className={classes.root}>
      <h2  >Nesto</h2>
      ...
      {(userType=='ADMIN' || userType=='seller') && (< SellerDelete
        open={isDialogOpen}
        id={delId}
